...
│   ├── permissions/      # Role/permission constants and checks (shared by core + services)
...
pnpm lint:deps            # Check module layering rules (core must not import apps/gateway, etc. — see .oxlintrc.json)
...
### Key Entry Points
- **Message flow**: channel webhook (`src/gateway/routes/webhooks/`) or websocket (`src/apps/channels/webchat/`) receives a guest message → `src/core/pipeline/index.ts` wires Butler-specific adapters (`src/core/pipeline/adapters.ts`) into `createPipeline` from `packages/pipeline` and runs the stage sequence
- **Task creation & escalation**: pipeline stages (e.g. `src/core/pipeline/stages/route-task.ts`) call into `src/services/task.ts` and the pipeline escalate stage to create tasks and escalate to staff
- **AI responses**: generated by the pipeline's `generate-response` stage (`packages/pipeline/src/stages/generate-response.ts`), using the AI provider bound in `src/core/pipeline/adapters.ts`
