...
│   │   ├── conversation/ # FSM, escalation, guest-context
...
│   │   └── pms/          # PMS reservation sync (providers live in packages/)
...
- **Task creation & escalation**: pipeline stages (e.g. `src/core/pipeline/stages/route-task.ts`) call into `src/core/conversation/` (`fsm.ts`, `escalation.ts`) and `src/services/task.ts` to create tasks and escalate to staff
