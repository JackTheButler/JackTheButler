/**
 * Hospitality intent catalog for the pipeline.
 *
 * Two responsibilities in one file:
 *   1. `IntentDefinitions` — the hotel-specific intents with descriptions,
 *      examples, and routing metadata (department, priority, identity
 *      requirements). Imported by `task-router` for routing decisions.
 *   2. `intentProvider` — the package's `IntentProvider` implementation,
...
/** Look up an intent's full definition by name. */
...
/**
 * Projects an `IntentDefinition` into the package's `Intent` shape with
 * routing metadata (department, requiresAction, requiresIdentity,
 * priority) folded onto `intent.metadata` so downstream stages and the
 * responder can read them without knowing the hospitality flavour.
 */
function toIntent(name: string, def: IntentDefinition): Intent {
  return {
    name,
    ...
    metadata: {
      ...
      requiresIdentity: def.requiresIdentity,
      priority: def.priority,
    },
  };
}

const cachedList: readonly Intent[] = Object.entries(IntentDefinitions).map(
  ([name, def]) => toIntent(name, def),
);

export const intentProvider: IntentProvider = {
  list: () => cachedList,
  get: (name) => {
    const def = getIntentDefinition(name);
    return def ? toIntent(name, def) : null;
  },
};
