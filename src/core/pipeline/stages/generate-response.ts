...
export async function generateResponse(ctx: MessageContext): Promise<void> {
  ...
  ctx.aiResponse = await defaultResponder.generate(ctx.conversation, ctx.inbound, ctx.guestContext, ctx.knowledgeResults, ctx.memories, ctx.classification, ctx.verification, ctx.conversationHistory);
