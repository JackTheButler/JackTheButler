/**
 * Hotel Profile Service
 *
 * Manages the hotel profile setting (stored as JSON under the 'hotel_profile'
 * settings key) plus static reference data (timezones, currencies, countries,
 * languages) used by the hotel profile UI.
 *
 * @module services/hotel-profile
 */

import { settingsService } from '@/services/settings.js';
import { createLogger } from '@/utils/logger.js';

const log = createLogger('hotel-profile');
...
export interface HotelProfileResult {
...
/**
 * IANA timezones for dropdown, sorted by UTC offset then alphabetically
 */
...
export class HotelProfileService {
  /**
   * Get the current hotel profile, falling back to defaults when unset.
   * Legacy profiles saved before `propertyLanguage` existed on the schema
   * are backfilled from the standalone `property_language` setting.
   */
  async getProfile(): Promise<HotelProfileResult> {
    const profile = await settingsService.get<HotelProfile | null>(SETTINGS_KEY, null);

    if (!profile) {
      return { profile: DEFAULT_PROFILE, isConfigured: false };
    }
  ...
  /**
   * Update the hotel profile. Dual-writes `propertyLanguage` as a standalone
   * `property_language` setting for the translation service to consume.
   */
  async updateProfile(profile: HotelProfile): Promise<HotelProfile> {
...
export const hotelProfileService = new HotelProfileService();
